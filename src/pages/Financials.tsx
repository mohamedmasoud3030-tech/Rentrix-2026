                                ...
                                {officeWorkspace.topOverdueInvoices.map((invoice) => (
                                    ...
                                    >
                                        <span className="min-w-0">
                                            ...
                                            <span className="block truncate text-xs text-slate-500 dark:text-slate-400">{invoice.unitName} Ã¢â‚¬Â¢ {formatDate(invoice.dueDate)}</span>
                                        </span>
                        ...
                        {officeWorkspace.maintenanceImpact.map((record) => (
                            ...
                            >
                                <span className="min-w-0">
                                    ...
                                    <span className="block truncate text-xs text-slate-500 dark:text-slate-400">{record.propertyName} Ã¢â‚¬Â¢ {record.unitName}</span>
                                </span>
    ...
    return (
        <div className="space-y-5">
                                    ...
                                    <Td className="text-left">
                                        <div className="flex justify-end">
                                            <ActionsMenu items={[
                                                ...
                                                VoidAction(() => requestConfirm('تأكيد إلغاء سند القبض؟', () => financeService.voidReceipt(r.id)))
                                            ]} />
                                        </div>
                                    </Td>
                                </Tr>
            ...
            <ConfirmDialog
                ...
                message={confirmState.message || 'ØªØ£ÙƒÙŠØ¯ Ø§Ù„Ø¥Ø¬Ø±Ø§Ø¡'}
                                ...
                                <Td className="text-left">
                                  <div className="flex justify-end">
                                    <ActionsMenu items={[
                                        ...
                                        VoidAction(() => requestConfirm('تأكيد إلغاء المصروف؟', () => financeService.voidExpense(e.id)))
                                    ]} />
                                  </div>
                                </Td>
                            </Tr>
                        ...
                        {db.depositTxs.map(tx => {
                            ...
                            return (
                                <Tr key={tx.id} className={tx.status === 'VOID' ? 'bg-slate-50/70 opacity-60 line-through' : ''}>
                                    ...
                                    <Td className="text-left">
                                      <div className="flex justify-end">
                                        {tx.status !== 'VOID' && <ActionsMenu items={[VoidAction(() => requestConfirm('تأكيد إلغاء حركة التأمين؟', () => financeService.voidDepositTx(tx.id)))]}/>}
                                      </div>
                                    </Td>
                                </Tr>
                            );
                        ...
                        {filtered.map(s => {
                            ...
                            return (
                                <Tr key={s.id} className={s.status === 'VOID' ? 'bg-slate-50/70 opacity-60 line-through' : ''}>
                                    ...
                                    <Td className="text-left">
                                      <div className="flex justify-end">
                                        <ActionsMenu items={[ EditAction(() => { setEditingSettlement(s); setIsModalOpen(true); }), VoidAction(() => requestConfirm('تأكيد إلغاء تسوية المالك؟', () => financeService.voidOwnerSettlement(s.id))) ]} />
                                      </div>
                                    </Td>
                                </Tr>
                            );
