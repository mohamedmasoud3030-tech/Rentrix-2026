...
const Financials: React.FC = () => {
    ...
    const runConfirm = async () => {
        if (!confirmState.action) {
            setConfirmState({ open: false, message: '', action: undefined, busy: false });
            return;
        }
        try {
    ...
    const officeWorkspace = useMemo(() => {
        ...
        const openMaintenance = db.maintenanceRecords.filter((record) =>
            ['NEW', 'OPEN', 'IN_PROGRESS'].includes(record.status)
        );
        ...
        const ownerPayables = Object.values(ownerBalances || {}).reduce(
            (sum: number, item: any) => sum + Number(item?.net || 0),
            0
        );
        const tenantReceivables = Object.values(contractBalances || {}).reduce(
            (sum: number, item: any) => sum + Math.max(Number(item?.balance || 0), 0),
            0
        );
    ...
    return (
        <div className="app-page page-enter">
            <PageHeader
                ...
                description="إدارة السندات، المصروفات، والتحويلات المالية للملاك والمستأجرين."
            />
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <Card className="p-5">
                    ...
                    <div className="mt-2 text-2xl font-extrabold text-slate-800 dark:text-slate-100">
                        {formatCurrency(officeWorkspace.receiptsTotal, currency)}
                    </div>
                    <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                        {officeWorkspace.receiptsCount.toLocaleString('ar')} سند قبض مرحّل
                    </div>
                </Card>
                ...
                <Card className="p-5">
                    ...
                    <div className="mt-2 text-2xl font-extrabold text-slate-800 dark:text-slate-100">
                        {formatCurrency(officeWorkspace.expensesTotal, currency)}
                    </div>
                    <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                        {officeWorkspace.expensesCount.toLocaleString('ar')} مصروف مرحّل
                    </div>
                </Card>
                ...
                <Card className="p-5">
                    ...
                    <div className="mt-2 text-2xl font-extrabold text-slate-800 dark:text-slate-100">
                        {formatCurrency(officeWorkspace.ownerPayables, currency)}
                    </div>
                    <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                        {officeWorkspace.settlementsCount.toLocaleString('ar')} تسوية مالك مرحّلة
                    </div>
                </Card>
                ...
                <Card className="p-5">
                    ...
                    <div className="mt-2 text-2xl font-extrabold text-slate-800 dark:text-slate-100">
                        {formatCurrency(officeWorkspace.tenantReceivables, currency)}
                    </div>
                    <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                        {officeWorkspace.overdueInvoices.length.toLocaleString('ar')} فاتورة متأخرة
                    </div>
                </Card>
            </div>
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-800/70">
                            ...
                            <div className="mt-3 space-y-2 text-sm text-slate-700 dark:text-slate-200">
                                <div>
                                    <strong>صافي التدفق:</strong>{' '}
                                    {formatCurrency(
                                        officeWorkspace.receiptsTotal -
                                            officeWorkspace.expensesTotal -
                                            officeWorkspace.settlementsTotal,
                                        currency
                                    )}
                                </div>
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
                                    ...
                                    >
                                        <span className="min-w-0">
                                            ...
                                            <span className="block truncate text-xs text-slate-500 dark:text-slate-400">
                                                {invoice.unitName} • {formatDate(invoice.dueDate)}
                                            </span>
                                        </span>
                                        <span className="font-extrabold text-rose-600 dark:text-rose-300">
                                            {formatCurrency(Number(invoice.amount || 0) + Number(invoice.taxAmount || 0), currency)}
                                        </span>
                                    </button>
                                ))}
                                {!officeWorkspace.topOverdueInvoices.length && (
                        ...
                        <div className={infoPanelCls}>
                            ...
                            <div className="space-y-2">
                                ...
                                {!officeWorkspace.topOwnerBalances.length && (
                                    <div className="text-sm text-slate-500 dark:text-slate-400">
                                        لا توجد أرصدة موجبة تحتاج تحويلًا الآن.
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        {officeWorkspace.maintenanceImpact.map((record) => (
                            ...
                            >
                                <span className="min-w-0">
                                    ...
                                    <span className="block truncate text-xs text-slate-500 dark:text-slate-400">
                                        {record.propertyName} • {record.unitName}
                                    </span>
                                </span>
                                <ArrowRightLeft size={15} className="text-slate-400" />
                            </button>
                        ))}
                        {!officeWorkspace.maintenanceImpact.length && (
                            <div className="text-sm text-slate-500 dark:text-slate-400">
                                لا توجد طلبات صيانة حرجة في الوقت الحالي.
                            </div>
                        )}
                    </div>
                </Card>
            ...
            <ConfirmDialog
                ...
                message={confirmState.message || 'تأكيد الإجراء'}
                onConfirm={runConfirm}
                onCancel={() => setConfirmState({ open: false, message: '', action: undefined, busy: false })}
            />
        </div>
    );
};
        ...
        <div className="space-y-5">
            ...
            {filteredReceipts.length ? (
                                        ...
                                        <div className="flex justify-end">
                                            <ActionsMenu
                                                items={[
                                                    ...
                                                    VoidAction(() =>
                                                        requestConfirm('تأكيد إلغاء سند القبض؟', () => financeService.voidReceipt(r.id))
                                                    ),
                                                ]}
                                            />
                                        </div>
                ...
                <EmptyState
                    ...
                    description="ابدأ بإضافة أول سند قبض أو جرّب تعديل كلمات البحث."
                />
            )}
...
const ExpensesView: React.FC<{ requestConfirm: ConfirmRequest }> = ({ requestConfirm }) => {
    const { db, financeService } = useApp();
    ...
    const filteredExpenses = useMemo(() => {
        return db.expenses
            .filter((e) => e.no.includes(searchTerm) || e.category.includes(searchTerm) || e.notes.includes(searchTerm))
            .sort((a, b) => new Date(b.dateTime).getTime() - new Date(a.dateTime).getTime());
        ...
        <div className="space-y-5">
            ...
            {filteredExpenses.length ? (
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {filteredExpenses.map((e) => (
                            <Tr key={e.id} className={e.status === 'VOID' ? 'bg-slate-50/70 opacity-60 line-through' : ''}>
                                ...
                                <Td>
                                    <StatusPill status={e.status}>{e.status === 'POSTED' ? 'مرحّل' : 'ملغي'}</StatusPill>
                                </Td>
                                <Td className="text-left">
                                    <div className="flex justify-end">
                                        <ActionsMenu
                                            items={[
                                                ...
                                                VoidAction(() =>
                                                    requestConfirm('تأكيد إلغاء المصروف؟', () => financeService.voidExpense(e.id))
                                                ),
                                            ]}
                                        />
                                    </div>
                ...
                <EmptyState
                    ...
                    description="لم يتم تسجيل أي مصروفات بعد لهذه الفترة."
                />
            )}
...
const DepositsView: React.FC<{ requestConfirm: ConfirmRequest }> = ({ requestConfirm }) => {
    const { db, financeService } = useApp();
    const [isModalOpen, setIsModalOpen] = useState(false);
    ...
    return (
        <div className="space-y-5">
            ...
            {db.depositTxs.length ? (
                <TableWrapper>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {db.depositTxs.map((tx) => {
                            ...
                            const tenant = contract ? db.tenants.find((t) => t.id === contract.tenantId) : null;
                            const typeMap = {
                                DEPOSIT_IN: 'إيداع جديد',
                                DEPOSIT_DEDUCT: 'خصم للإصلاح',
                                DEPOSIT_RETURN: 'إرجاع مستحقات',
                            } as const;
                            ...
                            return (
                                <Tr key={tx.id} className={tx.status === 'VOID' ? 'bg-slate-50/70 opacity-60 line-through' : ''}>
                                    ...
                                    <Td>
                                        <StatusPill status={tx.status}>{tx.status === 'POSTED' ? 'مرحّل' : 'ملغي'}</StatusPill>
                                    </Td>
                                    <Td className="text-left">
                                        <div className="flex justify-end">
                                            {tx.status !== 'VOID' && (
                                                <ActionsMenu
                                                    items={[
                                                        VoidAction(() =>
                                                            requestConfirm('تأكيد إلغاء حركة التأمين؟', () => financeService.voidDepositTx(tx.id))
                                                        ),
                                                    ]}
                                                />
                                            )}
                ...
                <EmptyState
                    icon={PiggyBank}
                    title="لا توجد حركات وديعة"
                    description="أضف أول حركة لتتبع مبالغ التأمين والخصومات المرتبطة بالعقود."
                />
            )}
...
const OwnerSettlementsView: React.FC<{ requestConfirm: ConfirmRequest }> = ({ requestConfirm }) => {
    const { db, financeService } = useApp();
    const [isModalOpen, setIsModalOpen] = useState(false);
    ...
    return (
        <div className="space-y-5">
            ...
            {filtered.length ? (
                        ...
                        {filtered.map((s) => {
                            ...
                            return (
                                <Tr key={s.id} className={s.status === 'VOID' ? 'bg-slate-50/70 opacity-60 line-through' : ''}>
                                    ...
                                    <Td>
                                        <StatusPill status={s.status}>{s.status === 'POSTED' ? 'مرحّل' : 'ملغي'}</StatusPill>
                                    </Td>
                                    <Td className="text-left">
                                        <div className="flex justify-end">
                                            <ActionsMenu
                                                items={[
                                                    ...
                                                    VoidAction(() =>
                                                        requestConfirm('تأكيد إلغاء تسوية المالك؟', () => financeService.voidOwnerSettlement(s.id))
                                                    ),
                                                ]}
                                            />
                                        </div>
                ...
                <EmptyState
                    icon={Landmark}
                    title="لا توجد تسويات ملاك"
                    description="عند تحويل المستحقات إلى المالك ستظهر هنا كل التسويات المالية."
                />
            )}
...
const EditReceiptForm: React.FC<{ isOpen: boolean; onClose: () => void; receipt: Receipt | null }> = ({
    isOpen,
    onClose,
    receipt,
}) => {
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={`تعديل سند قبض #${receipt.no}`}>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <p className="rounded-xl bg-blue-50 p-3 text-center text-xs text-blue-700">
                    لا يمكن تعديل المبلغ أو العقد لضمان سلامة القيود المحاسبية.
                </p>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className={labelCls}>التاريخ والوقت</label>
                        <input type="datetime-local" value={dateTime} onChange={(e) => setDateTime(e.target.value)} required className={inputCls} />
                    </div>
                    <div>
                        <label className={labelCls}>مرجع / رقم الحوالة</label>
                ...
                <div className="flex justify-end gap-2 pt-4 border-t">
                    ...
                    <button type="submit" className={primaryButtonCls}>حفظ التعديلات</button>
                </div>
            </form>
        </Modal>
    );
...
const ReceiptAllocationModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
    ...
    const totalAllocated = useMemo(
        () => Array.from(allocations.values()).reduce((sum: number, amount: number) => sum + amount, 0),
        [allocations]
    );
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="إضافة سند قبض وتخصيص الدفعات">
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {contractId && (
                    <div className="space-y-2">
                        <h3 className="border-b border-slate-100 pb-2 text-sm font-extrabold uppercase tracking-wider text-slate-700">
                            تخصيص على الفواتير المستحقة
                        </h3>
                        ...
                        {unpaidInvoices.length > 0 ? (
                            <div className="max-h-60 overflow-y-auto rounded-xl border border-slate-200 bg-white">
                                {unpaidInvoices.map((inv) => (
                                    <div
                                        key={inv.id}
                                        className="grid grid-cols-4 items-center gap-4 border-b border-slate-100 p-3 last:border-b-0 hover:bg-slate-50/70 transition-colors"
                                    >
                            ...
                            <p className="rounded-xl bg-slate-50 p-4 text-center text-sm text-slate-500">
                                لا توجد فواتير مستحقة لهذا العقد حاليًا.
                            </p>
                        )}

                        <div
                            className={`grid grid-cols-3 gap-2 rounded-xl border p-3 text-xs font-bold ${
                ...
                <div className="flex justify-end gap-3 border-t border-slate-100 pt-4">
                    ...
                    <button
                        type="submit"
                        className={`${primaryButtonCls} disabled:cursor-not-allowed disabled:opacity-50`}
                        disabled={!isBalanced || receiptData.amount <= 0}
                    >
                        حفظ السند
                    </button>
                </div>
...
const ExpenseForm: React.FC<{ isOpen: boolean; onClose: () => void; expense: Expense | null }> = ({
    isOpen,
    onClose,
    expense,
}) => {
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={expense ? 'تعديل مصروف' : 'إضافة مصروف جديد'}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className={labelCls}>يخصم من</label>
                        <select value={chargedTo} onChange={(e) => setChargedTo(e.target.value as any)} className={inputCls}>
                    ...
                    <div>
                        <label className={labelCls}>العقد المرتبط (اختياري)</label>
                        <select value={contractId || ''} onChange={(e) => setContractId(e.target.value || null)} className={inputCls}>
                ...
                <div className="flex justify-end gap-3 border-t border-slate-100 pt-4">
                    <button type="button" onClick={onClose} className={ghostButtonCls}>إلغاء</button>
                    <button type="submit" className={primaryButtonCls}>حفظ وتسجيل المصروف</button>
                </div>
            </form>
        </Modal>
    );
...
const DepositTxForm: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="حركة مبلغ تأمين (وديعة)">
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label className={labelCls}>العقد / المستأجر</label>
...
const OwnerSettlementForm: React.FC<{ isOpen: boolean; onClose: () => void; settlement: OwnerSettlement | null }> = ({
    isOpen,
    onClose,
    settlement,
}) => {
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={settlement ? 'تعديل تسوية المالك' : 'تحويل رصيد للمالك'}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    ...
                    <select value={ownerId} onChange={(e) => setOwnerId(e.target.value)} required className={inputCls}>
                        {db.owners.map((o) => (
