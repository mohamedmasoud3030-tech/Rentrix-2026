  ...
  return (
    <div className={`relative flex flex-col gap-4 overflow-hidden rounded-[28px] border ${c.border} bg-white/90 p-5 shadow-brand backdrop-blur-xl transition-all duration-200 hover:-translate-y-0.5 hover:shadow-brand-lg dark:bg-slate-900/88`}>
      ...
      <div className="flex items-start justify-between">
        ...
        {trend && (
          <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${trendUp ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-rose-50 text-rose-600 dark:bg-rose-500/10 dark:text-rose-300'}`}>
            {trend}
          </span>
        )}
      </div>
