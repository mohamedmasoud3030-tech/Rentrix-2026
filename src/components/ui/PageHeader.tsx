...
const PageHeader: React.FC<PageHeaderProps> = ({ title, description, icon, children }) => {
  return (
    <div className="mb-8 rounded-[30px] border border-slate-200/70 bg-white/70 px-5 py-5 shadow-sm backdrop-blur-sm dark:border-slate-800/80 dark:bg-slate-900/55 lg:flex lg:items-end lg:justify-between lg:gap-6 lg:px-6">
      ...
      {children && <div className="mt-5 flex flex-wrap items-stretch gap-2 self-start lg:mt-0 lg:self-auto">{children}</div>}
    </div>
  );
};
