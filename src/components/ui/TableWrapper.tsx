...
export const Th: React.FC<HeaderProps> = ({ className = '', children, sortable = false, sorted = null, ...props }) => (
  <th
    className={`sticky top-0 z-[1] hidden border-b border-slate-200/80 bg-slate-50/94 px-3 py-2 text-right text-[10px] font-extrabold tracking-[0.14em] text-slate-500 backdrop-blur-sm dark:border-slate-700 dark:bg-slate-800/94 dark:text-slate-400 sm:table-cell ${className}`}
    {...props}
  >
...
export const Tr: React.FC<React.HTMLAttributes<HTMLTableRowElement>> = ({ className = '', children, ...props }) => (
  <tr
    className={`mb-2 block overflow-hidden rounded-[16px] border border-slate-200/80 bg-white/92 shadow-sm transition-all duration-150 hover:bg-sky-50/40 focus:bg-sky-50/40 focus:outline-none dark:border-slate-800 dark:bg-slate-900/86 dark:hover:bg-slate-800/45 dark:focus:bg-slate-800/45 sm:mb-0 sm:table-row sm:rounded-none sm:border-0 sm:bg-transparent sm:shadow-none ${className}`}
    {...props}
  >
    {children}
  </tr>
...
const TableWrapper: React.FC<React.TableHTMLAttributes<HTMLTableElement>> = ({ className = '', children, ...props }) => {
  return (
    <div className="overflow-hidden rounded-[22px] border border-slate-200/80 bg-white/92 shadow-brand backdrop-blur-sm dark:border-slate-800 dark:bg-slate-900/90">
