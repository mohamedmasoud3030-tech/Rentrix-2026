...
const SummaryStatCard: React.FC<SummaryStatCardProps> = ({ label, title, value, icon, color = 'info', subtext }) => {
  ...
  return (
    <div className="group relative overflow-hidden rounded-[30px] border border-white/75 bg-white/90 p-5 shadow-brand backdrop-blur-xl transition-all duration-200 hover:-translate-y-0.5 hover:shadow-brand-lg dark:border-slate-800/90 dark:bg-slate-900/88">
      ...
      <div className="flex items-start gap-4">
        <div className={`flex h-12 w-12 items-center justify-center rounded-[18px] ${colorClasses[color]}`}>{icon}</div>
        <div className="min-w-0 flex-1">
          <p className="text-[11px] font-extrabold tracking-[0.16em] text-slate-500 dark:text-slate-400">{title || label}</p>
          <p className="mt-2 text-[1.7rem] font-black tracking-tight text-slate-900 dark:text-slate-50">{value}</p>
